    ...
    def _parse_flight_parameters(self):
        ...
        try:
            ...
            if missing:
                ...
                self.last_missing_fields = list(missing)
                self.last_validation_error = f"Missing required fields: {', '.join(missing)}"
                return None

            # Prepare params dict with all parameters
            params = fr.copy()  # Include all parameters, not just required ones

            # Normalize order so values like order=["101"] or order="101" are handled safely.
            order_raw = params.get("order", [])
            if isinstance(order_raw, str):
                try:
                    parsed = ast.literal_eval(order_raw)
                except Exception:
                    parsed = order_raw
                order_raw = parsed

            if isinstance(order_raw, (tuple, set)):
                order_raw = list(order_raw)
            ...
            for token in order_raw:
                ...
                if isinstance(token, str):
                    ...
                    if not t:
                        continue
                    normalized_order.append(t)
                elif isinstance(token, (int, np.integer)):
                    normalized_order.append(str(int(token)))
                elif isinstance(token, (float, np.floating)):
                    if float(token).is_integer():
                        normalized_order.append(str(int(token)))
                    else:
                        normalized_order.append(str(token))
                else:
                    normalized_order.append(str(token))

            if not normalized_order:
                self.last_validation_error = "Field 'order' is empty after parsing."
                return None

            params["order"] = normalized_order

            # Normalize standard route dictionary keys to strings for consistent lookup.
            std_routes = params.get("standard_flight_routes", {}) or {}
            if isinstance(std_routes, dict):
                params["standard_flight_routes"] = {str(k): v for k, v in std_routes.items()}
            else:
                self.last_validation_error = "Field 'standard_flight_routes' must be a dictionary."
                return None
        ...
        try:
            ...
            if left_groups:
                ...
                if left_waypoints and hasattr(self.app.visualizer, 'add_polyline'):
                    ...
                    debug_print(f"✅ Visualized Overview Flight Left: {len(left_waypoints)} total waypoints")
            ...
            if right_groups:
                ...
                if right_waypoints and hasattr(self.app.visualizer, 'add_polyline'):
                    ...
                    debug_print(f"✅ Visualized Overview Flight Right: {len(right_waypoints)} total waypoints")

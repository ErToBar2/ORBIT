...
def generate_underdeck_routes(app) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Ensure bridge_width is available (safety-critical). Compute robustly if missing/invalid.
        try:
            bw = float(params.get('bridge_width', 0.0))
        except Exception:
            bw = 0.0
        if bw <= 0.0:
            try:
                bw = _calculate_bridge_width_from_app(app)
                params['bridge_width'] = bw
                debug_print(f"✅ Resolved bridge_width from app context: {bw:.2f}m")
            except Exception as _e_bw:
                error_debug_print(f"❌ bridge_width is required for safe underdeck routes and could not be resolved: {_e_bw}")
                return []
...
def _parse_underdeck_parameters(app) -> Dict[str, Any]:
    ...
    params = {
        ...
        # IMPORTANT: do not inject arbitrary defaults; resolve robustly later
        'bridge_width': project_data.get('bridge_width', 0.0),
        ...
        'num_passes_middle': flight_data.get(
            'num_passes_middle',
            2 * flight_data.get('num_passes', 2)
        ),
        ...
        # Derived values
        'num_spans': len(flight_data.get('num_points', flight_data.get('horizontal_offsets_underdeck', [])))
    }
...
def _print_parameters(params: Dict[str, Any]):
    ...
    debug_print(f"   num_passes_middle: {params['num_passes_middle']}")
...
def _calculate_bridge_width_from_app(app) -> float:
    """Resolve bridge_width from app parsed data or cross-section.

    Returns
    -------
    float
        Positive bridge width in meters.
    Raises
    ------
    ValueError
        If bridge width cannot be determined.
    """
    try:
        # 1) Prefer parsed data
        if hasattr(app, 'parsed_data'):
            bw_raw = app.parsed_data.get('project', {}).get('bridge_width')
            debug_print(f"🔍 [UNDERDECK] Raw bridge_width from parsed data: {bw_raw} ({type(bw_raw)})")
            if bw_raw is not None:
                try:
                    # handle array-like
                    import numpy as _np  # local alias to avoid top-level dependency concerns
                    if isinstance(bw_raw, (list, tuple)) or ('ndarray' in str(type(bw_raw))):
                        if len(bw_raw) > 0:
                            bw = float(bw_raw[0])
                        else:
                    ...
                    debug_print(f"⚠️ [UNDERDECK] Failed to convert bridge_width: {conv_err}")

        # 2) Fallback: compute from cross-section transformed points
        if hasattr(app, 'crosssection_transformed_points') and app.crosssection_transformed_points is not None:
            try:
                pts = np.array(app.crosssection_transformed_points)
                if pts.size > 0 and pts.shape[1] >= 2:
                    bw = float(np.max(pts[:, 0]) - np.min(pts[:, 0]))
                    debug_print(f"📏 [UNDERDECK] Calculated bridge_width from cross-section: {bw:.2f}m")
                    if bw > 0:
                        return bw
            except Exception as e_cs:
                debug_print(f"⚠️ [UNDERDECK] Cross-section width calc failed: {e_cs}")

        raise ValueError("bridge_width not found; please provide it in project settings")
    except Exception as e:
        # Bubble up – caller decides whether to abort
        raise
...
def _generate_underdeck_routes_complete_logic(trajectory_samples: List, params: Dict[str, Any], app=None) -> List[Dict[str, Any]]:
    ...
    # Offset points (apply bridge_width/2 + user offset at point of use)
    total_offsets = [
        (params.get('bridge_width', 0.0) / 2.0) + float(x)
        for x in (params.get('horizontal_offsets_underdeck', []) or [])
    ]
    offset_points_underdeck = _compute_points_with_horizontal_offset(
        base_points, normals, total_offsets,
    ...
    if override_spans:
        ...
        for s in sorted(list(override_spans)):
            ...
            off_val_total = (params.get('bridge_width', 0.0) / 2.0) + float(off_val)
            debug_print(f"   ↳ Span {s+1}: using horizontal_offset={off_val_total:.2f}m with angle=0° (pillar-parallel)")
        ...
        try:
            ...
            for s in sorted(list(override_spans)):
                ...
                if repl:
                    ...
                    debug_print(f"   🔄 Span {s+1}: replaced with {len(repl)//2} pillar-parallel offset pairs from trajectory crossings @ ±{off_val_total:.2f}m")
        except Exception as e:
...
def _create_multipass_underdeck_routes(offset_points_underdeck: List[List], params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for span_idx, span_offset_points in enumerate(offset_points_underdeck):
        ...
        middle_passes = params.get('num_passes_middle', num_passes)
        if middle_passes is None:
            middle_passes = num_passes
        ...
        for base_idx in range(max(len(right_points), len(left_points))):
            ...
            passes_for_this_base = middle_passes if is_middle_odd else num_passes
        ...
        if route_points:
            total_passes = sum(
                middle_passes if (len(right_points) % 2 == 1 and i == len(right_points) // 2) else num_passes
            ...
            routes.append({
                ...
                'pattern': 'crossing',
                'num_passes_middle': params.get('num_passes_middle', params.get('num_passes', 2))
            })
...
def _fix_connection_speed_tags(routes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    When a connection tag is found on a point, this function:
      - overwrites the tag of the immediate previous point with the same
        connection tag for vertical hops only (same XY, different Z).

    This keeps both vertical legs (low→high and high→low) at connection speed
    while preserving the horizontal pass-through segment speed after returning
    to the low point.
    ...
    -------
    List[Dict[str, Any]]
        Routes with corrected connection speed tags for vertical segments
    """
    ...
    for route_idx, route in enumerate(routes):
        ...
        for i in range(1, len(points)):  # Start from index 1 to ensure there's a previous point
            ...
            if 'connection' in tag:
                ...
                if len(prev_point) >= 4:
                    # Only retag the previous point for true vertical connection hops
                    # to avoid leaking connection speed onto horizontal passes.
                    is_vertical_hop = (
                        abs(prev_point[0] - point[0]) <= 1e-6 and
                        abs(prev_point[1] - point[1]) <= 1e-6 and
                        abs(prev_point[2] - point[2]) > 1e-6
                    )
                    if not is_vertical_hop:
                        continue

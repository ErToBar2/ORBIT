...
from .context import VerticalRef
from .context import CoordinateSystemRegistry
...
class BridgeDataLoader:
    ...
    def _load_minimal_json(self, json_path: Path, context: Optional[ProjectContext]):
        ...
        import json
        import numpy as np
        from orbit.io.models import Trajectory, Bridge
        try:
            from  orbit.io.models import Pillar
        except Exception:
    ...
    def get_last_import_options(self) -> Dict[str, Any]:
        """Return the latest bridge-import dialog selections in a serializable format."""
        file_path = None
        try:
            if getattr(self, "last_selected_file", None):
                file_path = str(Path(self.last_selected_file))
        except Exception:
            file_path = None
    ...
    def apply_import_options(self, import_options: Optional[Dict[str, Any]]):
        """Apply persisted bridge-import options before loading."""
        if not import_options:
            return
        try:
            coord = import_options.get("coordinate_system")
            if coord:
    ...
    def load_bridge_data(self, selected_file: Optional[Path] = None, import_options: Optional[Dict[str, Any]] = None) -> Tuple[Optional[Bridge], Optional[ProjectContext]]:
        ...
        # Replay mode: seed the loader with previously stored dialog options
        self.apply_import_options(import_options)
        ...
        if selected_file is not None:
            ...
            if selected_path.exists() and selected_path.is_file():
                bridge_file = selected_path
                self.last_selected_file = selected_path
                debug_print(f"[DEBUG] Using provided bridge source (no dialog): {bridge_file}")
            else:
                debug_print(f"[DEBUG] Provided bridge source not found: {selected_path}")
    ...
    def _create_empty_bridge(self, name: str) -> Bridge:
        ...
        from orbit.io.models import Trajectory
        import numpy as np
    ...
    def _perform_cross_section_analysis(self, image_path: Path):
        ...
        try:
            ...
            try:
                ...
                from  orbit.gui.cross_section_analysis import process_crosssection_image
    ...
    def _select_template(self, template_path: Path):
        """Handle template selection - copy to project, then display & analyze the single, final file."""
        try:
            dest = self._copy_template_to_project_atomic(template_path)  # returns Path
            self.current_crosssection_path = str(dest)
            # Show & analyze strictly from the final destination file to avoid races
            self._display_cross_section_image(dest)
            self._perform_cross_section_analysis(dest)
        except Exception as e:
            debug_print(f"[DEBUG] Cross-section template handling failed: {e}")
            # Fallback: at least show the original template
            try:
                self._display_cross_section_image(template_path)
            except Exception:
                pass
    
    def _copy_template_to_project_atomic(self, template_path: Path) -> Path:
        """Atomically copy template into the project's input dir; return destination Path."""
        ...
        dest = import_dir / f"{self._sanitize_filename(bridge_name) or 'DefaultBridge'}_crosssection{template_path.suffix}"

        # If source==dest, nothing to do
        try:
            if template_path.resolve(strict=True) == dest.resolve(strict=False):
                debug_print("[COPY] Source and destination are the same; skipping copy")
                return dest
        except Exception:
            pass

        # Stream copy to a temp file, flush, then atomic replace
        with open(template_path, "rb") as src:
            fd, tmp_path = tempfile.mkstemp(prefix=dest.name + ".", dir=str(import_dir))
            try:
                with os.fdopen(fd, "wb", closefd=True) as tmp:
                    shutil.copyfileobj(src, tmp, length=1024 * 1024)  # 1MB chunks
                    tmp.flush()
                    os.fsync(tmp.fileno())
                os.replace(tmp_path, dest)  # atomic on modern Windows/Linux/macOS
            except Exception:
                # ensure tmp removed on error
                try: os.remove(tmp_path)
                except Exception: pass
                raise

        debug_print(f"[COPY] Done -> {dest}")
        return dest
    ...
    def _setup_project_structure(
        ...
        *,
        ...
        """
        try:
            from orbit.io.context import ProjectContext, VerticalRef
            import shutil
...
class FileSelectionDialog(QDialog):
    ...
    # Theme (unchanged)
        ...
        if hasattr(self, 'last_coord_system') and self.last_coord_system:
            default_coord = (
                CoordinateSystemRegistry.resolve_coordinate_system_key(self.last_coord_system)
                or self.last_coord_system
            )
        ...
        if default_index >= 0:
            ...
            fallback_key = CoordinateSystemRegistry.get_default_coordinate_system_key()
            fallback_index = self.coord_combo.findData(fallback_key)
            if fallback_index >= 0:
                self.coord_combo.setCurrentIndex(fallback_index)
        ...
        self.custom_epsg_input.setPlaceholderText(CoordinateSystemRegistry.get_custom_epsg_placeholder())

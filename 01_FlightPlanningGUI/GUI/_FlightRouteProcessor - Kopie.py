...
class UASPhotogrammetricFlightPathCalculator:
    def __init__(self, trajectory, normals, flight_route_offset_H_base, flight_route_offset_V_base, standard_flight_routes):
    ...
    def compute_flight_route(self, route_id, reverse=False, tag=None):
        ...
        for point, normal in zip(self.trajectory, self.normals):
            # Ensure the direction multiplier is correctly set for '1xx' routes (right) and '2xx' routes (left)
            direction_multiplier = 1 if route_id.startswith('1') else -1  # Default right for '1xx', left for '2xx'
            normal = np.array(normal) * direction_multiplier  # Apply the direction multiplier directly to the normal
            offset_point = point + normal * h_offset + np.array([0, 0, v_offset])  # Apply horizontal offset along modified normal
    ...
    def process_routes(self, order, pass_underdeck, underdeck_safe_flythrough):
        current_route = []
        right_side = []
        left_side = []
        ...
        for item in order:
            ...
            elif item == underdeck_safe_flythrough:
                underdeck_points = [[pt[0], pt[1], pt[2], "underdeck_safe_flythrough"] for pt in item]
                if pass_underdeck:

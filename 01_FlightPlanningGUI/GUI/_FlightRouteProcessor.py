...
class UnderdeckFlightRouteProcessor:
    ...
    def generate_back_and_forth_route(self, route_segment, connection_height):
        """
        Generate a back and forth route for surveying tasks with 2 passes and include vertical connections.
        """
        full_sequence = []
        for i in range(0, len(route_segment), 2):
            ...
            full_sequence.extend([left, right, right, left])

        # Add vertical connections at the specified points
        if len(full_sequence) > 1:
            new_sequence = []
            for index, point in enumerate(full_sequence):
                new_sequence.append(point)
                if index == 0 or index == 1 or index == len(full_sequence) - 2 or index == len(full_sequence) - 1:
                    new_z = point[2] + connection_height
                    modified_point = [point[0], point[1], new_z] + point[3:] if len(point) > 3 else [point[0], point[1], new_z]
                    new_sequence.append(modified_point)
                    new_sequence.append(point)
